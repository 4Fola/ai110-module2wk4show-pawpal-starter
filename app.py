...
# --- PHASE6-EXT: Auto-load JSON if present (safe; preserves comments) ---
try:
    if os.path.exists("data.json"):
        st.session_state.owner = Owner.load_from_json("data.json")
        owner = st.session_state.owner
except Exception:
    # If anything goes wrong, fall back to the in-memory Owner created above.
    pass
# -------------------------------
...
with schedule_tab:
    ...
    if st.button('Generate schedule'):
        ...
        if tasks:
            ...
            # --- PHASE6-EXT: Priority badges + colored background via pandas Styler ---
            import pandas as pd
